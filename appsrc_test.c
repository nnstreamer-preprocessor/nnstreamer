...
static void prepare_buffer(GstAppSrc* appsrc, char *index) {
  ...
  while(fgetc(fp) != EOF){
    ...
    if(cnt%4==0){
        //calculate     
        xy_theta=atan(y/x)*val+80;
        z_theta=atan(z/x)*val+9;
        //0<z_theta & z_theta<=64 &
        if( 0<z_theta & z_theta<=64 & 0< xy_theta & xy_theta<=160 ){
               
            range=round(sqrt(x*x+y*y+z*z)*100)/100;
            if(100<range & range<1){
                range=0;
            }
            xy_theta=round(xy_theta*4);
            
            //interpolation/2d projection 
